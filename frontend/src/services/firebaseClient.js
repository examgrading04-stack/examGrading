...
class MockUser {
  ...
  async updateProfile(data) {
    Object.assign(this, data);
    await fetch(`${BASE_URL}/api/db/users/${this.email}/profiles/${this.email}`, {
      method: "PUT",
  ...
  async reload() {
    const res = await fetch(`${BASE_URL}/api/db/users/${this.email}/profiles/${this.email}`);
    if (res.ok) {
    ...
    return new Promise((resolve, reject) => {
      try {
        if (typeof google === "undefined" || !google.accounts) {
          throw new Error("Google Identity Services SDK ยังโหลดไม่สำเร็จ กรุณาต่ออินเทอร์เน็ตเพื่อเข้าใช้งาน");
        }
        ...
        const client = google.accounts.oauth2.initTokenClient({
          ...
          callback: async (tokenResponse) => {
            if (tokenResponse.error) {
              reject(new Error(tokenResponse.error_description || "Google Popup closed or failed"));
              return;
            }
            if (tokenResponse.access_token) {
              try {
                ...
                if (!res.ok) {
                  const err = await res.json();
                  throw new Error(err.detail || "การยืนยัน Google Token ล้มเหลวจาก API");
                }
...
class MockCollectionReference {
  ...
  async get() {
    ...
    const res = await fetch(`${BASE_URL}/api/db/${this.path.join("/")}${queryStr}`);
    if (!res.ok) {
    ...
    const docs = (dataList || []).map(item => {
      ...
      if (item.user_email && this.path.length === 1 && ["exams", "students", "results", "subjects"].includes(this.path[0])) {
