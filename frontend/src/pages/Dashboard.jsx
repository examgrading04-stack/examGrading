...
export function Dashboard({ data }) {
  ...
  return (
    <div className="page-enter max-w-[1600px] mx-auto space-y-4 flex flex-col h-[calc(100vh-105px)] lg:h-[calc(100vh-137px)] overflow-hidden">
      {/* Stat Grid */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 min-h-0">
        {/* Chart Section */}
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col min-h-0">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-slate-800 truncate text-sm">
                          {res.studentName || res.studentCode || "ไม่ระบุผู้สอบ"}
                        </p>
                        <div className="flex items-center gap-1.5 mt-0.5 text-[11px] text-slate-500 truncate">
                          <span className={`font-semibold ${isFlagged ? "text-rose-600" : "text-emerald-600"}`}>{statusText}</span>
                          <span>•</span>
                      ...
                      <div className="text-right ml-3 flex flex-col items-end">
                        <p className="font-black text-slate-800 text-sm">
                          {res.score}<span className="text-slate-400 font-semibold text-xs">/{res.total}</span>
                        </p>
