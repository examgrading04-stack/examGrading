...
class BaseDBAdapter(ABC):
    ...
    @abstractmethod
    def get_students(
        self, user_email: str, student_ids: Optional[List[str]] = None
    ) -> List[dict[str, Any]]:
        pass
...
class SqlSystemLog(Base):
    ...
    userEmail = Column(
        ...
        primary_key=True,
    )  # Note: old frontend used userEmail, now maps to user_id
...
class SqlStudentEnrollment(Base):
    ...
    enrollment_id = Column(
        "enrollment_id", Integer, primary_key=True, autoincrement=True
    )
...
class SqlExamAnswerKey(Base):
    ...
    answer_key_id = Column(
        "answer_key_id", Integer, primary_key=True, autoincrement=True
    )
...
class MySQLAdapter(BaseDBAdapter):
    ...
    def _to_dict(self, model_obj, session=None) -> dict[str, Any]:
        ...
        d = {
            ...
            for c in getattr(model_obj, "__mapper__").column_attrs
        }
        ...
        try:
            ...
            if isinstance(model_obj, SqlResult):
                details = (
                    ...
                    .filter(
                        SqlExamDetail.result_id == model_obj.id,
                        SqlExamDetail.user_id == model_obj.user_email,
                    )
                    .order_by(SqlExamDetail.question_no)
                    .all()
                )
                answers_dict = {}
                wrong_list = []
                skipped_list = []
                for det in details:
                    answers_dict[str(det.question_no)] = (
                        det.student_answer if det.student_answer else ""
                    )
                ...
                student = (
                    ...
                    .filter(SqlStudent.id == model_obj.studentCode)
                    .first()
                )
                if student:
            ...
            if isinstance(model_obj, SqlExam):
                keys = (
                    ...
                    .filter(
                        SqlExamAnswerKey.exam_id == model_obj.id,
                        SqlExamAnswerKey.user_id == model_obj.user_email,
                    )
                    .order_by(SqlExamAnswerKey.question_no)
                    .all()
                )
                answer_key_dict = {}
                ...
                for key in keys:
                    if is_custom:
                        answer_key_dict[str(key.question_no)] = {
    ...
    def get_exam(self, user_email: str, exam_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not row:
                raise HTTPException(
                    status_code=404, detail=f"Exam not found: {exam_id}"
                )
            d = self._to_dict(row)
            ...
            subj = (
                ...
                .filter(SqlSubject.code == row.subject_id)
                .first()
            )
            if subj:
            ...
            if row.section_id:
                sec = (
                    ...
                    .filter(SqlSection.id == row.section_id)
                    .first()
                )
                if sec:
                    d["section"] = sec.sec
    ...
    def update_exam(self, user_email: str, exam_id: str, data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if answer_key and isinstance(answer_key, dict):
                session.query(SqlExamAnswerKey).filter(
...
>>>>>>> origin/main
            ...
            for r in rows:
                try:
                    ...
                    print(
                        f"Warning: skipping exam {getattr(r, 'id', '?')} due to error: {e}"
                    )
                    continue
            return res
        finally:
    ...
    def get_students(
        ...
        session = self._get_session()
        try:
            query = session.query(SqlStudent).filter(
            ...
            if section_ids:
                sections = (
                    session.query(SqlSection)
                    .filter(SqlSection.id.in_(section_ids))
                    .all()
                )
    ...
    def get_subject_name(self, user_email: str, subject_code: str) -> str:
        ...
        try:
            row = (
                session.query(SqlSubject)
                .filter(
                    SqlSubject.user_email == user_email, SqlSubject.code == subject_code
                )
                .first()
            )
    ...
    def save_result(self, user_email: str, payload: dict[str, Any]) -> str:
        ...
        try:
            ...
            st = (
                session.query(SqlStudent)
                .filter_by(id=student_code, user_email=user_email)
                .first()
            )
            if not st:
                st_name = (
                    ...
                    else f"นักเรียน {student_code}"
                )
                new_st = SqlStudent(
                    id=student_code, name=st_name, user_email=user_email
                )
                session.add(new_st)
                session.flush()
            ...
            if exam_id and student_code:
                existing = (
                    ...
                    .filter_by(examId=exam_id, studentCode=student_code)
                    .first()
                )
                if existing:
                    if not overwrite:
                        ...
                        session.query(SqlExamDetail).filter_by(
                            result_id=existing.id, user_id=user_email
                        ).delete()
            ...
            img_val = (
                data.get("imageUrl") or data.get("image_url") or data.get("imageURL")
            )
            if img_val:
    ...
    def get_doc(
        self, collection: str, doc_id: str, user_email: Optional[str] = None
    ) -> Optional[dict]:
        ...
        try:
            ...
            if hasattr(model_cls, "id"):
                ...
                query = query.filter(model_cls.user_id == doc_id)
            elif hasattr(model_cls, "template_id"):
                query = query.filter(model_cls.template_id == doc_id)
                ...
                if hasattr(model_cls, "username"):
                    ...
                    query = query.filter(
                        or_(model_cls.email == doc_id, model_cls.username == doc_id)
                    )
                else:
    ...
    def set_doc(
        self, collection: str, doc_id: str, user_email: Optional[str], data: dict
    ) -> None:
        model_cls = _get_model_class(collection)
        if not model_cls:
            return
        ...
        try:
            ...
            if collection in ("users", "profiles"):
                ...
                if "password" in mapped_data:
                    ...
                    if not (
                        isinstance(raw_pass, str)
                        and len(raw_pass) == 64
                        and all(c in "0123456789abcdef" for c in raw_pass.lower())
                    ):
                        mapped_data["password"] = hashlib.sha256(
                            raw_pass.encode()
                        ).hexdigest()
                    else:
            ...
            if row:
                ...
                if collection == "exams" and "answerKey" in mapped_data:
                    session.query(SqlExamAnswerKey).filter(
                        SqlExamAnswerKey.exam_id == doc_id,
                        SqlExamAnswerKey.user_id == user_email,
                    ).delete()
                ...
                if hasattr(model_cls, "id"):
                    ...
                    mapped_data["template_id"] = doc_id
                elif hasattr(model_cls, "email"):
                    ...
                    if k in valid_cols:
                        if isinstance(v, str) and (
                            ...
                            if k == "flagged" and isinstance(v, list):
                                ...
                                k in ("user_email", "user_id", "userEmail") and v == ""
                            ):
                                cleaned_data[k] = None
                            else:
                                cleaned_data[k] = v
...
>>>>>>> origin/main
                    if sec_row:
                        subj_id = (
                            subjectCode
                            if (subjectCode and str(subjectCode).strip())
                            else sec_row.subject
                        )
                        enroll = (
                            session.query(SqlStudentEnrollment)
                            .filter(
...
>>>>>>> origin/main
    ...
    def delete_doc(
        self, collection: str, doc_id: str, user_email: Optional[str]
    ) -> None:
        model_cls = _get_model_class(collection)
        if not model_cls:
            return
    ...
    def get_results(self, user_email: str) -> List[dict[str, Any]]:
        ...
        try:
            rows = (
                session.query(SqlResult)
                .filter(SqlResult.user_email == user_email)
                .all()
            )
            if not rows:
                return []
            ...
            if student_codes:
                students = (
                    session.query(SqlStudent)
                    .filter(SqlStudent.id.in_(student_codes))
                    .all()
                )
            ...
            for row in rows:
                try:
                    d = {
                        c.key: getattr(row, c.key) for c in row.__mapper__.column_attrs
                    }
                    ...
                    print(
                        f"Warning: skipping result {getattr(row, 'id', '?')} due to error: {e}"
                    )
                    continue
            return res
        finally:
