...
def register_fonts():
    ...
    font_bold_path = os.path.join(os.path.dirname(__file__), "..", "..", "fonts", "THSarabunNew-Bold.ttf")
...
def prepare_report_data(exam: dict, results: list, students: list, subject_name: str, sections: list) -> list:
    student_map = {s.get("id") or s.get("studentCode") or s.get("student_id"): s for s in students}
    section_map = {str(s.get("id")): (s.get("name") or s.get("sec") or s.get("section_name")) for s in (sections or [])}
    data = []
    ...
    for r in results:
        ...
        sec_id = student.get("section") or student.get("sec") or exam.get("section") or ""
...
def generate_excel_report(exam: dict, results: list, students: list, subject_name: str, sections: list) -> str:
...
def generate_pdf_report(exam: dict, results: list, students: list, subject_name: str, sections: list) -> str:
    data = prepare_report_data(exam, results, students, subject_name, sections)
    
    fd, temp_path = tempfile.mkstemp(suffix=".pdf")
    os.close(fd)
    
    doc = SimpleDocTemplate(temp_path, pagesize=A4, rightMargin=30, leftMargin=30, topMargin=30, bottomMargin=30)
    elements = []
    ...
    # Table data
    headers = ["รหัสผู้เรียน", "ชื่อ-นามสกุล", "วิชา", "กลุ่มเรียน", "คะแนนที่ได้", "คะแนนเต็ม", "เปอร์เซ็นต์"]
    table_data = [headers]
    for row in data:
        table_data.append([
            row["รหัสผู้เรียน"],
