...
def diag_qr(warped, report: DiagReport, debug_img):
    ...
    if meta.subject_code or meta.total_questions:
        report.qr_ok = True
        report.info.append(
            f"✅ QR decode สำเร็จ: tq={report.qr_data.get('tq','?')} sc={report.qr_data.get('sc','?')}"
        )
    else:
        report.qr_ok = False
        report.issues.append(
            "❌ QR decode ไม่ได้ — ตรวจสอบขนาด QR และ quiet zone รอบ QR"
        )
...
def diag_grid(warped, warped_gray, n_q, report: DiagReport, debug_img):
    ...
    if grid_ratio < 0.20:
        report.warnings.append(
            "⚠️ พื้นที่ Grid เล็กกว่าที่คาด — อาจ detect bubble ได้ไม่ครบ"
        )
    if grid_ratio > 0.65:
...
def diag_bubbles(warped, grid_rect, n_q, report: DiagReport, debug_img):
    ...
    for gi, group in enumerate(positions):
        if group is None:
            report.issues.append(
                f"❌ Group {gi}: detect bubble ไม่ได้ — bubble อาจเล็กเกินไปหรือ contrast ต่ำ"
            )
            continue
        xs, ys = group
        rpg = len(ys)
        ...
        report.info.append(
            f"   Group {gi}: {len(xs)} cols × {rpg} rows = {len(xs)*rpg} bubbles"
        )

        # ตรวจ column count
        if len(xs) != 5:
            report.warnings.append(
                f"⚠️ Group {gi}: พบ {len(xs)} columns แทน 5 — K-means อาจ cluster ผิด"
            )
...
def diag_fill_quality(
    raw_scores, report: DiagReport, debug_img, warped, grid_rect, positions
):
    if not raw_scores:
        return
    ...
    try:
        ...
        report.info.append(
            f"✅ Summary marks: filled≈{sm['filled_count']}/{n_q} blank={sm['blank_count']}/{n_q} thr={sm['fill_threshold']}"
        )
        if sm["suspicious"]:
            report.warnings.append(
                f"⚠️ suspicious {len(sm['suspicious'])} ข้อ (เช่น {', '.join('Q'+str(x['question']) for x in sm['suspicious'][:10])})"
            )
    except Exception as e:
        ...
        for gi, group in enumerate(positions):
            ...
            for row, y in enumerate(ys):
                ...
                for ch, (x, ratio) in enumerate(zip(current_xs, ratios)):
                    ...
                    cv2.circle(
                        debug_img, (cx, cy), OMRConfig.BUBBLE_RADIUS - 2, color, -1
                    )
...
def add_legend(img):
    ...
    for i, (color, text) in enumerate(legends):
        ...
        cv2.putText(
            img, text, (x0 + 28, y + 4), cv2.FONT_HERSHEY_SIMPLEX, 0.55, WHITE, 2
        )
        cv2.putText(
            img, text, (x0 + 28, y + 4), cv2.FONT_HERSHEY_SIMPLEX, 0.55, BLACK, 1
        )
...
def diagnose(image_path: str, force_q: int = 0):
    ...
    if W_orig < 800 or H_orig < 800:
        report.warnings.append(
            f"⚠️ ภาพเล็กเกิน ({W_orig}×{H_orig}) — ควร ≥ 1000×1300 px"
        )
...
def main() -> None:
    ...
    if not clean:
        print(
            "Usage: python diagnose_sheet.py <image> [image2 ...] [--force-q 50] [--analyze-drift]"
        )
        ...
        if analyze_drift and r.anchor_ok and r.n_questions:
            try:
                ...
                if anchors is not None:
                    ...
                    ranked = sorted(
                        [x for x in rep if x["max_dist_px"] is not None],
                        key=lambda x: x["max_dist_px"],
                        reverse=True,
                    )
                    for it in ranked[:10]:
                        print(
                            f"  group={it['group']} row={it['row']:>3}: mean={it['mean_dist_px']:.2f}px max={it['max_dist_px']:.2f}px found={it['found']}/{it['expected']}"
                        )
            except Exception as e:
    ...
    for r in reports:
        status = (
            "✅"
            if (r.anchor_ok and r.qr_ok and not r.issues)
            else "⚠️" if r.warnings else "❌"
        )
        print(
            f"  {status} {r.path}: anchor={'OK' if r.anchor_ok else 'FAIL'} "
