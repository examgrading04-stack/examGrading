...
class _ScanScreenState extends State<ScanScreen> {
  ...
  Future<void> _uploadAndProcess() async {
    ...
    try {
      ...
      // 2. Call FastAPI
      final fastApiUrl = const String.fromEnvironment(
        'FASTAPI_URL',
        defaultValue: 'http://10.160.252.49:8000/',
      );

      final response = await http
          .post(
            Uri.parse('$fastApiUrl/api/scan-cloudinary'),
          ...
          .timeout(
            const Duration(seconds: 30),
            onTimeout: () =>
                throw Exception('หมดเวลาเชื่อมต่อกับ Server (Timeout)'),
          );
