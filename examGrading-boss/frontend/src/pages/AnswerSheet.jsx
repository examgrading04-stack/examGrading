                ...
                <div className="flex gap-4">
                      ...
                      <div className="flex gap-[2px]">
                        {Array.from({ length: 13 }, (_, col) => (
                          <div key={col} className="flex flex-col items-center">
                            ...
                            {Array.from({ length: 10 }, (_, num) => <div key={num} className="w-[10px] h-[10px] rounded-full border border-zinc-400 text-[5.5px] flex items-center justify-center mb-[1.5px] text-zinc-500 bg-white font-bold">{num}</div>)}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {setsCount > 1 && (
                    <div className="text-center">
                      ...
                      {Array.from({ length: setsCount }, (_, index) => <div key={index} className="w-[13px] h-[13px] rounded-full border border-zinc-400 text-[7px] flex items-center justify-center mb-0.5 text-zinc-600 bg-white font-bold">{String.fromCharCode(65 + index)}</div>)}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className={`${Number(sheet.questions) <= 50 ? "omr-columns-2" : "omr-columns-4"} h-full`}>
                {questionNumbers.map((question) => (
                  <div key={question} className="flex items-center gap-1 mb-[5px] break-inside-avoid">
                    <span className="w-[18px] text-right font-bold text-zinc-800 text-[9px] shrink-0">{question}.</span>
                    <div className="flex gap-[3px]">
                      {options.map((option) => <div key={option} className="w-[15px] h-[15px] rounded-full border border-zinc-500 flex items-center justify-center text-[7px] text-zinc-500 font-bold bg-white">{option}</div>)}
                    </div>
                  </div>
                ))}
              </div>
